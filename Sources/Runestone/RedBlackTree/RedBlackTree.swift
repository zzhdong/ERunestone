...
final class RedBlackTree<NodeID: RedBlackTreeNodeID, NodeValue: RedBlackTreeNodeValue, NodeData> {
    ...
    func node(containingLocation location: NodeValue) -> Node? {
        let safeLocation = min(max(location, minimumValue), root.nodeTotalValue)
    
        #if DEBUG
        if safeLocation != location {
            assertionFailure(
                "\(location) is out of bounds. Clamped to \(safeLocation). " +
                "Valid range is \(minimumValue) - \(root.nodeTotalValue)."
            )
        }
        #endif
    
        return node(
            containingLocation: safeLocation,
