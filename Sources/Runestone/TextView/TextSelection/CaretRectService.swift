...
final class CaretRectService {
    ...
    func caretRect(at location: Int, allowMovingCaretToNextLineFragment: Bool) -> CGRect {
        ...
        if allowMovingCaretToNextLineFragment &&
            shouldMoveCaretToNextLineFragment(forLocation: lineLocalLocation, in: line) {
            let nextLocation = min(safeLocation + 1, stringView.string.length)
            let rect = caretRect(at: nextLocation, allowMovingCaretToNextLineFragment: false)
...
private extension CaretRectService {
    private func shouldMoveCaretToNextLineFragment(forLocation location: Int, in line: DocumentLineNode) -> Bool {
        ...
        let safeLocation = min(max(location, 0), line.data.length)

        guard let lineFragmentNode = lineController.lineFragmentNode(containingCharacterAt: safeLocation) else {
            return false
        }
        guard lineFragmentNode.index > 0 else {
            return false
        }
        return safeLocation == lineFragmentNode.data.lineFragment?.range.location
    }
}
